...
const YourWallets = () => {
  ...
  // ------------------------
  return (
    <div className="h-full w-full grid grid-rows-[130px_auto] justify-items-center overflow-y-scroll">
      {/* Header with account name and Add Account button */}
      <div className="flex w-auto py-3 justify-between gap-20">
      ...
      <div className="gap-10 scroll-smooth w-[350px] sm:w-[500px] md:w-[600px] lg:w-[1000px]">
        {account.map(
          (acc, index) =>
            Number(accID) === acc.accountNumber &&
            Object.entries(acc.accountDetails).map(([walletType, walletDetails]) => (
              <div key={`${index}-${walletType}`} className="relative h-auto w-auto overflow-hidden z-50 mb-10">
                ...
                {/* Wallet Card */}
                <div className="h-auto rounded-2xl backdrop-blur-xl border border-blue-400/30 shadow-lg shadow-black/40 p-5 sm:p-10">
                  {/* Wallet Name and Icon with Add Balance btn */}
                  <div className="flex justify-between items-center mb-5">
                    {/* Wallet Name and Icon */}
                    <div className="text-l md:text-2xl text-[#dcdcdc] font-bold px-3 flex items-center gap-2 md:gap-3">
                      <img src={walletDetails.walletIconLocation} alt="" className="h-7 w-7 md:h-10 md:w-10" />
                      {walletDetails.walletName}
                    </div>
                    {selectedComponent !== walletDetails.privateKey && (
                      <button
                        className="h-8 w-auto bg-blue-500 text-white px-3 rounded-xl text-[10px] md:text-sm font-medium transition duration-300 transform hover:scale-110 cursor-pointer"
                        onClick={() => {
...
const WalletCredentials = ({ walletDetails, selectedComponent }: { walletDetails: Wallet; selectedComponent: string }) => {
  ...
  return selectedComponent === walletDetails.privateKey ? (
    ...
    <div className="h-auto border border-blue-400/30 rounded-2xl text-[#dcdcdc] bg-blue-500/15 flex flex-col justify-center p-5 md:p-10 gap-5 md:gap-10">
      <h1 className="font-medium text-sm md:text-xl font-poppins text-white/80">Wallet Credentials</h1>
      <div className="flex flex-col gap-4">
        {/* Public Key */}
        <div>
          <div className="flex justify-between px-1 md:text-sm text-[10px]">
            <h1 className="text-white/80 mb-2">PUBLIC KEY</h1>
            <h1 className="text-white/80 mb-2">COPY</h1>
          </div>
          <h1 className="text-md text-[#ebebebcc] p-2 px-4 rounded-[10px] bg-[#06122c] font-roboto">{walletDetails.publicKey.substring(0, 7)}...</h1>
        </div>

        {/* Private Key */}
        <div>
          <div className="flex justify-between px-1 md:text-sm text-[10px]">
            <h1 className="text-white/80 sm:mb-2">PRIVATE KEY</h1>
            <h1 className="text-white/80 mb-2">COPY</h1>
          </div>
          <h1 className="text-md text-[#ebebebcc] p-2 px-4 rounded-[10px] bg-[#06122c] font-roboto">{walletDetails.privateKey.substring(0, 7)}...</h1>
        </div>
      </div>
    </div>
  );
